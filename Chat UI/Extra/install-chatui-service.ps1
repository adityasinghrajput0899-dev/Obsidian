# PowerShell script to install Chat UI as Windows Service
$serviceName = "ObsidianChatUI"
$serviceDisplayName = "Obsidian Chat UI Server"
$serviceDescription = "Obsidian AI Chat UI Server running on port 3001"
$nodePath = "node.exe"

# Get the directory where this script is located
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$scriptPath = Join-Path $scriptDir "backend\server.js"
$workingDirectory = Join-Path $scriptDir "backend"

# Verify paths exist
if (-not (Test-Path $scriptPath)) {
    Write-Host "ERROR: Server.js not found at: $scriptPath" -ForegroundColor Red
    Write-Host "Please make sure the backend directory exists." -ForegroundColor Red
    exit 1
}
